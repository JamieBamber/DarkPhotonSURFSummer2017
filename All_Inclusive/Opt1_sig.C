...
/*
##########
Script to produce a 2D Histogram of the all inclusive signal weighted selected events, after applying cuts of:
*	|eta|<1.44 
*	100<MT<130
*	no isolated leptons
...
void Delphes::Loop(Float_t PT0=50, Float_t E0=50, Float_t MT_UP=130, Float_t MT_LW=100, Bool_t Iso_Leptons=true, Float_t Eta0=1.44,Float_t HPT0=0)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   //
   //Define Histogram 
   TH2F* h_sig = new TH2F("h_sig", "", nbins, 0, nbins*bin_width, nbins, 0, nbins*bin_width);
   //
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		for(Int_t i=1; i<=nbins+1; i++) {
			if (i <= E_min_bin) {
				for(Int_t j=1; j<=nbins+1; j++) {
					if (j <= PT_min_bin) {
						h_sig->SetBinContent(i,j,h_sig->GetBinContent(i,j)+Event_weight);
					}
				}
			}
		}
		...
		if (((jentry+1)*100) % nentries == 0) {
     	 ...
     	 printf("\r%.0f%% complete Sig, MT_UP = %.0f, MT_LW = %.0f",percent, MT_UP, MT_LW);
     	 cout.flush();
		}
   }
   //
   ...
   //
   TFile *hfile = new TFile("Opt_1.root", "UPDATE");
   h_sig->Write();
   hfile->Close();
}
